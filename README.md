...
Windows에서는 install.ps1, macOS와 Linux에서는 install.sh를 사용해서
설치 후 필요한 Codex 재시작과 hook 신뢰 방법도 안내해줘.
```

Codex가 저장소를 확인하고 운영체제에 맞는 설치기를 선택한 뒤, 설치 명령을
실행하기 전에 필요한 권한을 요청합니다.
...
요구 사항: Codex CLI 0.133 이상, `python` 명령으로 실행 가능한 Python 3.10 이상,
...
| 취소 | tool 취소 시 전체 child process tree 종료 |
...
Codex and review the required hooks after installation. Use install.ps1 on
Windows and install.sh on macOS or Linux.
```

Codex inspects the repository, selects the installer for your operating
system, and requests permission before running it.
...
Requirements: Codex CLI 0.133+, Python 3.10+ available as the `python`
command, and Windows, macOS, or Linux.
...
| Cancellation | Cancelling the tool terminates the complete child process tree |
